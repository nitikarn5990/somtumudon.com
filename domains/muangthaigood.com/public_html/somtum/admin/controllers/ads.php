    ...
    while ($row = $db->FetchArray($query)) {
        ?>
                                <tr>
                                    ...
                                    <td class="center"  width=""><img src="<?php echo $ads_files->getDataDescLastID("file_name", "ads_id = '" . $row['id'] . "'") == "" ? NO_IMAGE : ADDRESS_ADS . $ads_files->getDataDescLastID("file_name", "ads_id = '" . $row['id'] . "'") ?>" style="height:70px; width:70px;"></td>
                                    ...
                                    <td class="center"  width=""><a href="<?php echo ADDRESS_ADMIN_CONTROL ?>ads&action=edit&id=<?php echo $row['id'] ?>" class="btn btn-primary btn-small">แก้ไข / ดู</a> <a href="#" onclick="if (confirm('คุณต้องการลบข้อมูลนี้หรือใม่?') == true) {
                                                document.location.href = '<?php echo ADDRESS_ADMIN_CONTROL ?>ads&action=del&id=<?php echo $row['id'] ?>'
